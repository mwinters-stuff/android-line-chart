...
public class ScorecardView extends View
{
  ...
  private Path createPath()
  {
    ...
    float mBottomEdge = (f1 + mFingerPosition.y);
    Path localPath = new Path();
    List<ScorecardOrientation> localList = computeOrientation();
    ScorecardOrientation mOrientation;
    if (localList.size() == 0)
    {
      mOrientation = mAvailableOrientations[0];
    }else
    {
      mOrientation = localList.get(0);
    }
    Log.d("CHART","Orientation " + mOrientation.name());
    switch (mOrientation)
    {
  ...
  List<ScorecardOrientation> computeOrientation()
  {
    float f1 = mTextLines.length * mTextPaint.getTextSize() / 2.0F + mLabelPadding;
    float f2 = mMaxTextWidth / 2.0F + mLabelPadding;
    ArrayList<ScorecardOrientation> localArrayList = new ArrayList<ScorecardOrientation>(Arrays.asList(mAvailableOrientations));
